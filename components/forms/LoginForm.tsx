...
const LoginForm = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        const result = await postLogin(identifier, password).then(r => console.log('data', r));
            ...
            <div>
                ...
                <div className="mt-1 relative rounded-md shadow-sm">
                    <input
                        ...
                        required
                        className={`appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-600 text-gray-900 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm`}/>
                    <div className={classNames(locale.isRTL ? `left-0` : `right-0` , `absolute inset-y-0 px-3 flex items-center`)}>
                        <EyeIcon className="h-5 w-5 text-gray-500" aria-hidden="true" onClick={() => setShowPassword(!showPassword)}/>
                    </div>
                </div>
            </div>
